<?php
  /**
   * Fecha:  2020-01-14 - Update: 2020-01-14
   * PHP Version 7
   * 
   * @category   Components
   * @package    Moodle
...
$red = optional_param('red', '', PARAM_TEXT); 
$area_adscripcion = optional_param('area_adscripcion', '', PARAM_TEXT); 
$ubicacion = optional_param('ubicacion', '', PARAM_TEXT); 
...
$baseurl = new moodle_url('/mod/iteasyexam/iteasyexam.php', $params);
...
$PAGE->set_url($CFG->wwwroot.'/mod/iteasyexam/iteasyexam.php');
...
if (($err_valida == '1') || ($err_valida == '2')) {
    ...
    if (is_null($id)) {
        $full_name = iteasyexam_Get_Full_name($USER->id); 
        $email = iteasyexam_Get_email($USER->id); 
    }
}
...
if (isloggedin()) {
    ...
    $PAGE->navbar->add(
        get_string('newupdate', 'iteasyexam'),
        $baseurl
    );
    ...
    echo $PAGE->set_title(get_string('newupdate', 'iteasyexam'));  
    echo $OUTPUT->header();  
    ...
    if ($id_course == '') {
        ...
        if ($mformst->is_cancelled()) {
            ...
            if ($validate_index == 0) {
                if ($data->course != '') {
                    $preregister = iteasyexam_Save_update(
                        $data
                    );
            ...
            if ($validate_index != 0) {
                ...
                if ($id_course != '') {
                    $course = iteasyexam_Get_Name_course($id_course);
                ...
                echo "</table>";
                echo "<br>".get_string('message_instructions_upd', 'iteasyexam').
                "<br><br>";
